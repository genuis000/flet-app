...
def main(page: ft.Page):
    ...
    fam_col = ft.Column([ck_fam_none, ck_fat, ck_hbp, ck_dm, ck_hlip, ck_gout], spacing=5)
    ...
    sport_col = ft.Column([ck_run, ck_ball, ck_rope, ck_swim, ck_power, ck_nosport], spacing=5)
    ...
    ill_col = ft.Column([ck_dizzy, ck_pale, ck_joint, ck_dry, ck_nosick], spacing=5)
    ...
    need_col = ft.Column([ck_1,ck_2,ck_3,ck_4,ck_5,ck_6,ck_7,ck_other], spacing=5)
    other_need = ft.TextField(label="其他需求填写", hint_text="请详细描述你的需求")
    ck_lim1=ft.Checkbox("无");ck_lim2=ft.Checkbox("素食");ck_lim3=ft.Checkbox("食物过敏");ck_lim4=ft.Checkbox("忌口食物")
    limit_cks=[ck_lim1,ck_lim2,ck_lim3,ck_lim4];bind_single_checkbox(limit_cks)
    limit_detail = ft.TextField(label="过敏/忌口详情说明", hint_text="请列出具体食物名称")

    report_text = ft.Text("个性化营养健康报告将在此生成", selectable=True, size=13, color=ft.Colors.GREY_600)
    loading_text = ft.Text("", color=ft.Colors.BLUE_500)
    ...
    def submit_all(e):
        ...
        if errors:
            page.show_snack_bar(ft.SnackBar(ft.Text("\n".join(errors)), bgcolor=ft.Colors.RED_300))
            return
        loading_text.value = "AI正在分析问卷数据，请等待..."
        page.update()
        try:
            fam_str = get_mul([ck_fam_none,ck_fat,ck_hbp,ck_dm,ck_hlip,ck_gout])
            sport_str = get_mul([ck_run, ck_ball, ck_rope, ck_swim, ck_power, ck_nosport])
            ill_str = get_mul([ck_dizzy, ck_pale, ck_joint, ck_dry, ck_nosick])
            need_str = get_mul([ck_1,ck_2,ck_3,ck_4,ck_5,ck_6,ck_7,ck_other])
            info = f"""
【一、基础信息】
性别：{get_text(sex_cks)}，年龄：{age.value}岁，年级：{grade.value}
身高：{height.value}cm，体重：{weight.value}kg
家族病史：{fam_str}
【二、膳食习惯】
早餐：{get_text(breakfast_cks)}，午晚餐：{get_text(lunch_cks)}
蔬菜：{get_text(veg_cks)}，水果：{get_text(fruit_cks)}，奶制品：{get_text(milk_cks)}
畜禽肉：{get_text(meat_cks)}，鱼虾：{get_text(fish_cks)}，鸡蛋：{get_text(egg_cks)}
膨化零食：{get_text(snack_cks)}，含糖饮料：{get_text(drink_cks)}，油炸烧烤：{get_text(fry_cks)}
主食：{get_text(staple_cks)}，口味：{get_text(taste_cks)}，吃饭速度：{get_text(speed_cks)}，宵夜：{get_text(night_cks)}
【三、运动作息】
周运动次数：{get_text(sportweek_cks)}，单次时长：{get_text(sporttime_cks)}，运动类型：{sport_str}
每日静态时长：{get_text(static_cks)}，日常零碎活动：{get_text(walk_cks)}
【四、睡眠情绪】
上床时间：{get_text(bed_cks)}，睡眠时长：{get_text(sleeph_cks)}，睡眠质量：{get_text(sleepq_cks)}
压力暴食：{get_text(stress_cks)}
【五、身体健康】
精力：{get_text(energy_cks)}，肠胃：{get_text(stomach_cks)}，三月体重变化：{get_text(weightc_cks)}
视力：{get_text(eye_cks)}，身体不适：{ill_str}
【六、改善诉求】
想要改善：{need_str}，其他需求：{other_need.value or "无"}
饮食禁忌：{get_text(limit_cks)}，禁忌详情：{limit_detail.value or "无"}
            """
            res = get_nutrition_report(info)
            report_text.value = res
            loading_text.value = "✅ 测评完成！"
        except Exception as e:
            report_text.value = f"❌ 生成报告时出错：{str(e)}"
            loading_text.value = "❌ 处理失败"
        page.update()

    content_col = ft.Column(spacing=15, controls=[
        ft.Text("青少年营养健康综合测评问卷", size=22, weight=ft.FontWeight.BOLD, color=ft.Colors.GREEN_700),
        ft.Divider(color=ft.Colors.GREEN_200),
        ft.Text("一、基础个人信息", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.BLUE_700),
        ft.Text("性别"),ft.Row(sex_cks, spacing=20),
        ft.Row([age, height], spacing=10),
        ft.Row([weight, grade], spacing=10),
        ft.Text("家族病史（可多选）"),fam_col,
        ft.Divider(),
        ft.Text("二、膳食饮食习惯", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.ORANGE_700),
        ft.Text("早餐进食频率"),ft.Row(breakfast_cks, spacing=15),
        ft.Text("午晚餐定时就餐"),ft.Row(lunch_cks, spacing=15),
        ft.Text("新鲜蔬菜每周频次"),ft.Row(veg_cks, spacing=10),
        ft.Text("新鲜水果每周频次"),ft.Row(fruit_cks, spacing=10),
        ft.Text("奶/酸奶每周频次"),ft.Row(milk_cks, spacing=10),
        ft.Text("畜禽肉类每周频次"),ft.Row(meat_cks, spacing=10),
        ft.Text("鱼虾水产"),ft.Row(fish_cks, spacing=15),
        ft.Text("鸡蛋食用"),ft.Row(egg_cks, spacing=15),
        ft.Text("膨化/糖果糕点"),ft.Row(snack_cks, spacing=15),
        ft.Text("含糖饮料"),ft.Row(drink_cks, spacing=15),
        ft.Text("油炸烧烤"),ft.Row(fry_cks, spacing=15),
        ft.Text("主食偏好"),ft.Row(staple_cks, spacing=15),
        ft.Text("口味偏好"),ft.Row(taste_cks, spacing=15),
        ft.Text("进食速度"),ft.Row(speed_cks, spacing=15),
        ft.Text("宵夜"),ft.Row(night_cks, spacing=15),
        ft.Divider(),
        ft.Text("三、运动与日常活动", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.PURPLE_700),
        ft.Text("每周主动运动次数"),ft.Row(sportweek_cks, spacing=15),
        ft.Text("单次运动时长"),ft.Row(sporttime_cks, spacing=15),
        ft.Text("运动类型（可多选）"),sport_col,
        ft.Text("每日静态时长(作业+电子产品)"),ft.Row(static_cks, spacing=15),
        ft.Text("日常零碎活动(爬楼步行)"),ft.Row(walk_cks, spacing=15),
        ft.Divider(),
        ft.Text("四、睡眠与情绪状态", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.TEAL_700),
        ft.Text("上床时间"),ft.Row(bed_cks, spacing=15),
        ft.Text("每日睡眠时长"),ft.Row(sleeph_cks, spacing=15),
        ft.Text("睡眠质量"),ft.Row(sleepq_cks, spacing=15),
        ft.Text("情绪压力暴饮暴食"),ft.Row(stress_cks, spacing=15),
        ft.Divider(),
        ft.Text("五、身体健康状况", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.RED_700),
        ft.Text("自我精力状态"),ft.Row(energy_cks, spacing=15),
        ft.Text("肠胃状况"),ft.Row(stomach_cks, spacing=15),
        ft.Text("近三月体重"),ft.Row(weightc_cks, spacing=15),
        ft.Text("视力"),ft.Row(eye_cks, spacing=15),
        ft.Text("身体不适（可多选）"),ill_col,
        ft.Divider(),
        ft.Text("六、个性化改善需求", size=16, weight=ft.FontWeight.BOLD, color=ft.Colors.INDIGO_700),
        ft.Text("目前想解决的问题（可多选）"),need_col,
        other_need,
        ft.Text("饮食禁忌"),ft.Row(limit_cks, spacing=15),
        limit_detail,
        ft.FilledButton("提交问卷，生成专属营养报告", on_click=submit_all, bgcolor=ft.Colors.GREEN_500, width=500),
        loading_text,
        ft.Divider(height=20),
        ft.Text("📄 个性化测评报告结果", size=16, weight=ft.FontWeight.BOLD),
        report_text
    ])
    page.add(ft.ListView(expand=True, controls=[content_col], padding=10))
